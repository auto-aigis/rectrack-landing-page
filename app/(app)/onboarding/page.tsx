...
const EMOJI: Record<string, string> = { Pickleball: '🏓', Soccer: '⚽', Softball: '🥎', Basketball: '🏀', Volleyball: '🏐', Tennis: '🎾' };
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    if (user?.onboarding_complete) { router.replace('/dashboard'); return; }
    onboardingApi.getSports().then(({ sports }) => setSports(sports)).catch(() => {});
  }, [user, router]);

  const sportData = sports.find((s) => s.name === selectedSport);
        ...
        {step === 1 ? (
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {sports.map((s) => (
                <button key={s.name} onClick={() => { setSelectedSport(s.name); setSelectedPosition(''); setStep(2); }}
                  className={`p-4 rounded-xl border-2 text-center transition hover:border-green-500 ${selectedSport === s.name ? 'border-green-600 bg-green-50' : 'border-gray-200 bg-white'}`}>
                  <div className="text-3xl mb-1">{EMOJI[s.name] || '🏅'}</div>
          ...
          <div>
            ...
            <h2 className="text-lg font-semibold text-gray-900 mb-1">{EMOJI[selectedSport] || '🏅'} {selectedSport}</h2>
            ...
            <div className="space-y-2">
              {sportData?.positions.map((pos) => (
                <button key={pos} onClick={() => setSelectedPosition(pos)}
                  className={`w-full p-3.5 rounded-xl border-2 text-left text-sm font-medium transition ${selectedPosition === pos ? 'border-green-600 bg-green-50 text-green-800' : 'border-gray-200 bg-white text-gray-700 hover:border-green-400'}`}>
                  {pos}
                </button>
              ))}
            </div>
            <Button onClick={handleSave} disabled={!selectedPosition || saving} className="w-full mt-6 bg-green-600 hover:bg-green-700 h-12 text-base">
